...
void sdsdisplqy(sds s) {
    struct sdshdr *sh = (void *) (s - (sizeof(struct sdshdr)));
    printf("sds infomation: len: %d free: %d buf: %s\n", sh->len, sh->free, sh->buf);
}
...
int main(void) {
    ...
    test_cond("sdsnew Create a string and obtain the length",
              sdslen(x) == 3 && memcmp(x, "foo\0", 4) == 0)

    sdsfree(x);
    x = sdsnewlen("foo", 2);
    test_cond("sdsnewlen Create a string with specified length",
              sdslen(x) == 2 && memcmp(x, "fo\0", 3) == 0)

    x = sdscat(x, "bar");
    test_cond("sdscat Strings concatenation",
              sdslen(x) == 5 && memcmp(x, "fobar\0", 6) == 0);

    sdsdisplqy(x);
//    printf("Allocated size: %ld\n", sdsAllocSize(x)); return 19, len(5)+free(5)+1+sizeof(struct sdshdr)(8) = 19
    sdsRemoveFreeSpace(x);
    sdsdisplqy(x);


    y = sdsnew("y");
    sdscatsds(x, y);
    test_cond("sdscatsds Sds concatenation",
              memcmp(x, "fobary\0", 7) == 0);

    x = sdscpy(x, "a");
    test_cond("sdscpy() against an originally longer string",
              sdslen(x) == 1 && memcmp(x, "a\0", 2) == 0)

    x = sdscpy(x, "xizxxxxxxxxxxyyyyyyyyyykkkkkkkkkk");
    test_cond("sdscpy() against an originally shorter string",
              sdslen(x) == 33 &&
              memcmp(x, "xizxxxxxxxxxxyyyyyyyyyykkkkkkkkkk\0", 33) == 0)
...
//test cmp end
    ...
    sdstoupper(y);
    test_cond("sdstoupper", memcmp(y, "BAR\0", 4) == 0);
    sdstolower(y);
    test_cond("sdstoupper", memcmp(y, "bar\0", 4) == 0);
